<html>
    ...
    <div class="ac">
    <form action="rcustomer.php" method="post">
        <div><b>Enter the Details to Retrive Customer:</b></div><br>
...
{ 
	...
	if($id!="" && $ifsc!="")  
	{ 
        $sql1 = "select * from customer where cusid='$id' and ifsc='$ifsc'";
        
        $result = mysqli_query($con,$sql1);
        if(mysqli_num_rows($result)>0){
        echo "<table><tr><th>cusid</th><th>fname</th><th>lname</th><th>email</th><th>address</th><th>ifsc</th></tr>";
        while($row = mysqli_fetch_assoc($result)){
            echo "<tr><td>".$row["cusid"]."</td><td>".$row["fname"]."</td><td>".$row["lname"]."</td><td>".$row["email"]."</td>
            <td>".$row["address"]."</td><td>".$row["ifsc"]."</td></tr>";
        }
        echo "</table>";
            ...
            echo "<p style='padding:20px; border-radius:5px; margin:20px; font-size:15px;margin-right:800px; border:1px solid rgba(255,0,0,10); background-color:rgba(255,50,50,0.3); color:red;'><b>Error:</b>$id,$ifsc is not Exist!</p>";
        }
	} else 
	{ 
