<html>
    ...
    <div class="ac">
    <form action="rmaintains.php" method="post">
        <div><b>Enter the Details to Retrive Maintains:</b></div><br>
   Account Number: <input type="number" name="accno" placeholder="Enter the Account No" required><br><br>ID: <input type="number" name="id" placeholder="Enter the Customer ID" required><br><br>
...
{ 
	...
	if($accno!="" && $id!="")  
	{ 
        $sql1 = "select * from maintains where accno='$accno' and cusid='$id'";
        
        $result = mysqli_query($con,$sql1);
        if(mysqli_num_rows($result)>0){
        echo "<table><tr><th>accno</th><th>cusid</th></tr>";
        while($row = mysqli_fetch_assoc($result)){
            echo "<tr><td>".$row["accno"]."</td><td>".$row["cusid"]."</td></tr>";
        }
        echo "</table>";
            ...
            echo "<p style='padding:20px; border-radius:5px; margin:20px; font-size:15px;margin-right:800px; border:1px solid rgba(255,0,0,10); background-color:rgba(255,50,50,0.3); color:red;'><b>Error:</b>$accno is not Exist!</p>";
        }
	} else 
	{ 
