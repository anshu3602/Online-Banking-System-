<html>
    ...
    <form action="daccount.php" method="post">
        <div><b>Enter the Details to Delete Account:</b></div><br>
   Account Number: <input type="number" name="accno" placeholder="Enter the Account No" required>
...
$con = mysqli_connect('localhost','root','','project')     or die("<p style='padding:20px; border-radius:5px; margin:20px; font-size:15px;margin-right:800px; border:1px solid rgba(255,0,0,10); background-color:rgba(255,50,50,0.3); color:red;'><b>Error:</b>Unable to selectdatabase: ".    mysqli_error($con))."</p>";
	$accno=$_POST['accno'];
     $ifsc = $_POST['ifsc'];
        ...
        $sql1 = "select * from account where accno='$accno' and ifsc='$ifsc'";
	...
	{ 
        ...
        {
            echo "<table><tr><th>accno</th><th>atype</th><th>balance</th><th>ifsc</th></tr>";
            while($row = mysqli_fetch_assoc($result))
            {
                echo "<tr><td>".$row["accno"]."</td><td>".$row["atype"]."</td><td>".$row["balance"]."</td><td>".$row["ifsc"]."</td></tr>";
            }
            echo "</table>";       

            $sql3 = "delete from account where accno = '$accno' and ifsc='$ifsc'";
        ...
        {
            echo "<p style='padding:20px; border-radius:5px; margin:20px; font-size:15px;margin-right:800px; border:1px solid red; background-color:rgba(255,50,50,0.3); color:red;'><b>Error:</b>$accno,$ifsc is not Exist!</p>";
        }
	}
    else 
	{ 
		echo "<p style='padding:20px; border-radius:5px; margin:20px; font-size:15px;margin-right:800px; border:1px solid rgba(255,0,0,10); background-color:rgba(255,50,50,0.3); color:red;'><b>Error:</b>".mysqli_error($con)."</p>"; 
	} 
 mysqli_close($con); 
 }
?>